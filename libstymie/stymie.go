...
func configure(p Plugin) error {
	return p.Configure()
}

func decrypt(p Plugin, b []byte) ([]byte, error) {
	return p.Decrypt(b)
}

func encrypt(p Plugin, b []byte) ([]byte, error) {
	return p.Encrypt(b)
}
...
func (s *Stymie) Configure() error {
	...
	// `Plugin.Configure` is optional, it could be a no-op.
